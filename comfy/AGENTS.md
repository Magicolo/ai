...
comfy/                          # wrapper repo (this file's scope)
  ...
  Zoomy/                        # zoomy app (see §10, full design in Zoomy/DESIGN.md)
    ...
    tests/                      # one test module per source module (114 tests: examples + Hypothesis properties, all green)
...
- **zoomy** (`Zoomy/`, full design in `Zoomy/DESIGN.md`) is the default interface: Gradio control panel (family dropdown → LoRA multi-select + strength sliders → Render next frame / Loop frames with optional target → Finalize video) driving `LocalEngine` in-process (diffusers frames + RIFE interp + ACE-Step music + MMAudio SFX, no backend), with a live stats line (frames · disk · last/avg durations · video · VRAM/RAM), recent-frames gallery, session log, graceful loop stop (uncheck the Loop box, finishes the frame) + forceful Interrupt. Families `ernie_turbo` / `z_fast` / `z_quality` (z pair shares sequence `z_image`); new family = one `FamilyDefinition` appended, nothing else changes. Existing workflow JSONs are untouched (additive).
...
- **Quality gates run after each completed task, not just at the end**: the full bind-mount loop (`ruff check --fix` + `ruff format` + `ruff check` + `mypy` + `pytest`) must pass before moving on; a task is done only when its gates are green.
...
# render_next_frame/finalize_video) via `docker exec -e PYTHONPATH=/application <zoomy-cid>`,
# never committed to the tree; frame contract is (family, prompt, negative_prompt,
# frame_count, lora_selections, seed)
...
- zoomy e2e writes real artifacts under `Zoomy/output/` (`Zoomy/<sequence>/frame_*` + `Zoomy_*`) — clean test renders from inside the zoomy container after verification (`rm -f /output/cold_* /output/ab2_* /output/*.flac` + stray sequence frames); never delete the user's `Ernie_Zoom_*`/`Z_Zoom_*` files by pattern accident (`Zoomy_` ≠ `Z_Zoom_`/`Ernie_Zoom_`).
