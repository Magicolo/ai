...
comfy/                          # wrapper repo (this file's scope)
  ...
  Zoomy/                        # zoomy app (see §10, full design in Zoomy/DESIGN.md)
    ...
    tests/                      # one test module per source module (132 tests: examples + Hypothesis properties, all green)
...
- **zoomy** (`Zoomy/`, full design in `Zoomy/DESIGN.md`) is the default interface: Gradio control panel (family dropdown → LoRA multi-select + strength sliders → Render next frame / Loop frames with target duration + width/height → Finalize video) driving `LocalEngine` in-process (diffusers frames + RIFE interp + ACE-Step music + MMAudio SFX, no backend), with a live stats line (frames · disk · last/avg durations · video · VRAM/RAM), recent-frames gallery, session log, graceful loop stop (uncheck the Loop box, finishes the frame) + forceful Interrupt. Loop = `generate_video` (duration → frame count via `(frames-1)*4+1` interp math; blank duration = unlimited until stopped; finalize-on-stop checkbox defaults on), so UI and programmatic generation can never drift. Per-request frame size (16-aligned, default 1376×768); flat output layout (`<sequence>/frame_*.png`, `<sequence>_*.mp4` + `-audio` twin). Every fallible engine stage auto-retries OOM (3 attempts, evict + empty-cache; interrupts/config propagate). Families `ernie_turbo` / `z_fast` / `z_quality` (z pair shares sequence `z_image`); new family = one `FamilyDefinition` appended, nothing else changes. Existing workflow JSONs are untouched (additive).
