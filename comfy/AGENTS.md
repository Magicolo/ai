...
- zoomy e2e writes real artifacts under `Zoomy/output/` (`<sequence>/frame_*` + `<sequence>_*`, flat since 2026-09-16 — no `Zoomy/` nesting, no `Zoomy_` prefix) — clean test renders from inside the zoomy container after verification (`rm -f /output/cold_* /output/ab2_* /output/*.flac` + stray sequence frames); never delete the user's `Ernie_Zoom_*`/`Z_Zoom_*` files by pattern accident.
