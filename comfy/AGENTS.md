...
comfy/                          # wrapper repo (this file's scope)
  ...
  Zoomy/                        # zoomy app (see §10, full design in Zoomy/DESIGN.md)
    ...
    tests/                      # one test module per source module (102 tests: examples + Hypothesis properties, all green)
...
- zoomy finalize: the `-audio.mp4` twin (not the same-named mp4) carries the soundtrack — preview/ship the twin. Audio floor is `max(interp/32, 1.0)` because `EmptyAceStep1.5LatentAudio.seconds` rejects anything below 1.0 with a 400. Long sequences finalize segmented (48-frame windows, per-stem FLAC stems + Python ffmpeg assembly — full design in `Zoomy/DESIGN.md` §7): a single-graph finalize OOMs MMAudio past ~200 frames, so NEVER "simplify" back to one pass; VRAM per segment peaks ~13.8/15.57 GiB. Never use `acrossfade` for segment joins (collapses on short tails — manual fades/delays instead); every assembly step verifies non-empty outputs. VHS also writes preview PNGs per segment — `remove_segment_files` covers all `seg*` extensions.
...
- zoomy poll loop scans history status messages for terminal failures BEFORE checking the `completed` flag — interrupted/errored prompts keep `completed: False` forever (verified live), so flag-only polling hangs until timeout.
